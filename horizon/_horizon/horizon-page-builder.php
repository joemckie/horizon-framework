...
function horizon_blog_output( $blog ) {
	...
	if ( (string) $blog_atts['enable_pagination'] == "No" ) {
		$args['nopaging'] = true;
	} else {
		$args['paged'] = get_query_var( 'paged' );
	}
	if ( $blog->category != "All" ) {
		$args['category_name'] = (string) $blog_atts['category'];
	}
...
function horizon_portfolio_output( $portfolio ) {
	...
	if ( $portfolio_atts['enable_pagination'] == "No" ) {
		$args['nopaging'] = true;
	} else {
...
function horizon_post_slider_output( $post_slider ) {
	...
	$args = array( 'posts_per_page' => $num );
	if ( $post_slider->category != "All" ) {
		$args['category_name'] = (string) $post_slider_atts['category'];
	}
...
function horizon_testimonials_output( $testimonial ) {
	...
	if ( $testimonial_query->found_posts < 2 ) {
		$testimonial_atts['type'] = 'static';
	} else {
		$testimonial_atts['type'] = "flow";
...
function horizon_build_saved_element( $saved_element, $element_type ) {
	...
	foreach ( $clone_meta_boxes['page-builder']['Page Builder']['elements'][$element_type] as $element => $args ) {
		...
		if ( $element == "acc_item" || $element == "tab_item" || $element == "toggle_item" ) {
			switch ( $element_type ) {
				case "Accordion":
					$html .= horizon_build_sub_items( $saved_element, $element_type, 'acc_item' );
					break;
				case "Tabs":
					$html .= horizon_build_sub_items( $saved_element, $element_type, 'tab_item' );
					break;
				case "Toggle":
					$html .= horizon_build_sub_items( $saved_element, $element_type, 'toggle_item' );
					break;
				default:
					break;
			}
...
function horizon_page_builder_output( $xml, $area ) {
	...
	if ( $area == "front-end" ) {
		foreach ( $xml as $node ) {
			...
			switch ( $node_name ):
				case "Accordion":
					$output .= horizon_accordion_output( $node );
					break;
				case "Blog":
					$output .= horizon_blog_output( $node );
					break;
				case "Call-To-Action":
					$output .= horizon_call_to_action_output( $node );
					break;
				case "Column":
					$output .= horizon_column_output( $node );
					break;
				case "Column-Service":
					$output .= horizon_column_service_output( $node );
					break;
				case "Contact-Form":
					$output .= horizon_contact_form_output( $node );
					break;
				case "Content":
					$output .= horizon_content_output( $node );
					break;
				case "Divider":
					$output .= horizon_divider_output( $node );
					break;
				case "Full-Width-Banner":
					$output .= horizon_full_width_banner_output( $node );
					break;
				case "Gallery":
					$output .= horizon_gallery_output( $node );
					break;
				case "Message-Box":
					$output .= horizon_message_output( $node );
					break;
				case "Portfolio":
					$output .= horizon_portfolio_output( $node );
					break;
				case "Page-Content":
					$output .= horizon_content_output( $node );
					break;
				case "Price-Table":
					$output .= horizon_price_table_output( $node );
					break;
				case "Post-Slider":
					$output .= horizon_post_slider_output( $node );
					break;
				case "Section-Start":
					$output .= horizon_section_start_output( $node );
					break;
				case "Section-End":
					$output .= horizon_section_end_output( $node );
					break;
				case "Sidebar":
					$output .= horizon_sidebar_output( $node );
					break;
				case "Staff":
					$output .= horizon_staff_output( $node );
					break;
				case "Tabs":
					$output .= horizon_tabs_output( $node );
					break;
				case "Testimonial":
					$output .= horizon_testimonials_output( $node );
					break;
				case "Toggle":
					$output .= horizon_toggle_output( $node );
					break;
				default:
					break;
			endswitch;
