...
if ( isset( $_POST['submitted'] ) ) {
	...
	if ( trim( $_POST['page-builder-contact-email'] ) === '' ) {
		...
		if ( !preg_match( "/^[[:alnum:]][a-z0-9_.-]*@[a-z0-9.-]+\.[a-z]{2,4}$/i", trim( $_POST['page-builder-contact-email'] ) ) ) {
...
<form action="<?php echo get_permalink(); ?>" class="horizon-form" id="page-builder-contact-form" method="post">
	...
	<ul>
		...
		<li>
			<div class="input-wrap">
				<input type="text" name="page-builder-contact-email" placeholder="Email" id="page-builder-contact-email"
					   value=""/>
				<span class="required">*</span>
			</div>
		</li>
		<li>
			<div class="input-wrap">
				<textarea name="page-builder-contact-message" id="page-builder-contact-message" placeholder="Message"
